...
- **WHEN** a Telegram user sends a direct or group message to `@news_desk_ai_bot`
- **THEN** the application-owned bot replies with the numeric Telegram chat ID
- **AND** the reply tells the user to paste that value into the Delivery dashboard's Telegram Chat ID field
- **AND** the dashboard still collects only the chat identifier, not bot tokens or webhook secrets
...
- **WHEN** the user selects Telegram delivery in the Delivery Channels dashboard tab
- **THEN** the dashboard identifies the application-owned bot as `@news_desk_ai_bot`
- **AND** it explains that the bot replies with the numeric chat ID for the selected direct or group chat
