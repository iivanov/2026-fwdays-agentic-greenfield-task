...
- **WHEN** cleanup runs with content rows older than seven days and run metadata newer than 30 days
...
Cleanup MUST keep unresolved operational failures visible and delete only resolved operational metadata after 30 days.
...
Cleanup MUST delete only closed stale integration circuits and MUST retain open or half-open circuit state.

#### Scenario: Active circuit is stale
- **WHEN** an `integration_circuits` row is `open` or `half_open` and older than 30 days
- **THEN** cleanup retains the row.
