...
- **THEN** GitHub Actions runs the committed root scripts for typechecking, linting, formatting, unit tests, backend coverage, browser build, Deno check, Deno lint, Deno format check, Deno dependency update check, Playwright smoke tests, actionlint, npm audit, and Supabase migration linting
