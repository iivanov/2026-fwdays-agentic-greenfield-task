...
## Purpose
Define the delivery worker behavior that turns persisted digests into durable,
ID-only delivery attempts and sends them through in-app, Brevo email, Telegram,
Slack, and generic signed webhook adapters with outbound safety, bounded
timeouts, retry/backoff, and circuit-breaker state.
